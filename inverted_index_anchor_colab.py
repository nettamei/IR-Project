...
class sim_calc:
    ...
    def get_title_binary_score(index, tokens):
        title_binary_score = {}
        for token in tokens:
            if index._posting_list.get(token) is not None:
                for doc_id, tf in index._posting_list[token]:
                    if doc_id in title_binary_score:
                        title_binary_score[doc_id] += tf
                    else:
                        title_binary_score[doc_id] = tf
        most_rel_by_title = sorted([(doc_id, score) for doc_id, score in title_binary_score.items()], key=lambda x: x[1], reverse=True)
