...
def search():
    ...
    if BM25_weight != 0 and cosin_weight == 0:
        rank_res_BM25 = sim_calc.BM25(stemmed_query_tokens, index_body,index_body_sorted_posting_list, BM25_weight, norm_pv)[:400]
        if len(stemmed_query_tokens) == 1:
            bm25_id_score = [(doc_id, score) for doc_id, score in rank_res_BM25]
            # bm25_id_score = sim_calc.z_score_normalize(rank_res_BM25)
        else:
            max_BM25_value = rank_res_BM25[0][1]
            # min_BM25_value = rank_res_BM25[len(rank_res_BM25) - 1][1]
            # bm25_id_score = [(doc_id, (score - min_BM25_value) / (max_BM25_value - min_BM25_value)) for doc_id, score in rank_res_BM25]
    ...
    if anchor_weight != 0:
        ...
        if len(stemmed_query_tokens) == 1:
            ...
            # anchor_id_score = sim_calc.z_score_normalize(rank_res_anchor)
            max_anchor_value = rank_res_anchor[0][1]
            # min_anchor_value = rank_res_anchor[len(rank_res_anchor) - 1][1]
    ...
    if title_weight != 0:
        ...
        # title_id_score = sim_calc.z_score_normalize(rank_res_title)
        max_title_value = rank_res_title[0][1]
        # min_title_value = rank_res_title[len(rank_res_title) - 1][1]
        # title_id_score = [(doc_id, (score - min_title_value) / (max_title_value - min_title_value)) for doc_id, score in rank_res_title]
    ...
    for i in range(0, 100):
        ...
        # res.append((str(final_doc_score[i][0]), final_doc_score[i][1], duration))
