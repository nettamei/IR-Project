...
anchor_URL = 'index_posting_locs_anchor_text_norm/index_anchor_text_norm.pkl'
...
def search():
    ...
    if cosin_weight != 0 and BM25_weight == 0:
        ...
        # print(f'cosin time: {time() - t_start}')
    ...
    if BM25_weight != 0 and cosin_weight == 0:
        t_start = time()
        rank_res_BM25 = sim_calc.BM25(stemmed_query_tokens, index_body, BM25_weight)[:400]
        # print(f'bm25 time: {time() - t_start}')
        # Normalize scores with the max score
    ...
    if anchor_weight != 0:
        ...
        # print(f'anchor time: {time() - t_start}')
        # Normalize scores with the max score
    ...
    if title_weight != 0:
        t_start = time()
        rank_res_title = sim_calc.get_title_binary_score(index_title_not_stemmed, query_tokens, title_weight)[:400]
        # print(f'title time: {time() - t_start}')
